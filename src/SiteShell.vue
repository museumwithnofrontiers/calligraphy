...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('calligraphy.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('calligraphy.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('calligraphy.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
